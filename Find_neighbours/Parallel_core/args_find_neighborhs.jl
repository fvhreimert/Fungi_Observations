function distance_fn(x1, y1, x2, y2)
  return haversine((x1, y1), (x2, y2)) # default radius is earth -- should be ok
end
...
function compare_x(array, extra_array, to_compare, i)
  ...
  return haversine(our_point, other_point) # default radius is earth -- should be ok
end
...
# df = CSV.read("./data/large_dataset_oct_30.csv", DataFrame)
#1442756
...
# subset_df = df_by_x[under_threshold_index_x, :]
# subset_df = sort(df_by_x, [:decimalLongitude])
...
for index_to_compare in arg1:arg2 
  ...
  # Calculate output for relevant, and retty print it
